...
CHANGES v1.4.0: Always retire old decision in _supersede(), even with empty text
CHANGES v1.3.0: Remove notes from DELTA_SCHEMA
CHANGES v1.1.0-v1.2.0: SOW v3.2.0 compliance, empty text handling
    ...
    "properties": {
        ...
        "ops": {"type": "array", "items": {
            ...
            "properties": {
                ...
                "category": {"type": "string"}, "owner": {"type": "string"},
...
def make_empty_summary(channel_id):
    ...
    return {
        "schema_version": "1.0", "channel_id": str(channel_id),
        "updated_at": datetime.now(timezone.utc).isoformat(),
        "summary_token_count": 0, "participants": [], "overview": "",
        "active_topics": [], "decisions": [], "key_facts": [],
        "action_items": [], "open_questions": [], "pinned_memory": [],
        "meta": {
            "model": "", "summarized_at": "", "token_count": 0,
            "message_range": {"first_id": 0, "last_id": 0, "count": 0},
            "verification": {
                "protected_items_count": 0, "hashes_verified": 0,
                "mismatches": 0, "source_checks_passed": 0,
...
def apply_ops(current, delta):
    """Apply delta ops[] to current summary. Returns deep copy."""
    summary = copy.deepcopy(current)
    summary["updated_at"] = datetime.now(timezone.utc).isoformat()
    for op in delta.get("ops", []):
        t, oid = op.get("op", ""), op.get("id", "")
        if t == "noop": continue
            ...
            if not any(p["id"] == oid for p in summary["participants"]):
                summary["participants"].append({
                    "id": oid,
                    "display_name": op.get("text") or op.get("title") or oid,
                })
        elif t == "add_topic":
        ...
        elif t == "update_topic_status":
            _update_status(summary["active_topics"], oid, op.get("status"))
        elif t == "add_decision":
            text = op.get("text", "")
        ...
        elif t == "supersede_decision":
            _supersede(summary["decisions"], op)
        elif t == "add_fact":
            text = op.get("text", "")
            ...
            _add_if_new(summary["action_items"], oid, {
                ...
                "status": op.get("status", "open"), "owner": op.get("owner"),
                "deadline": op.get("deadline"),
                "source_message_ids": op.get("source_message_ids", []),
            })
        elif t == "complete_action_item":
            _update_status(summary["action_items"], oid, "completed")
        ...
        elif t == "close_open_question":
            _update_status(summary["open_questions"], oid, "answered")
...
def _supersede(decisions, op):
    """Retire old decision, optionally create new one.
    Always retires old decision even if text is empty — the new
    decision entry typically comes from a separate add_decision op."""
    ...
    for d in decisions:
        if d["id"] == old_id:
            ...
            break
    if not found:
        logger.warning(f"supersede_decision: old id={old_id} not found")
    text = op.get("text", "").strip()
    if not text:
        logger.info(
            f"supersede_decision: retired id={old_id}, "
            f"no new entry (empty text)")
        return
    decisions.append({
        "id": op.get("id"), "decision": text,
...
def verify_protected_hashes(candidate, snapshot):
    ...
    _HASH_FIELDS = [
        ("decisions", "decision"), ("key_facts", "fact"),
        ("action_items", "task"), ("pinned_memory", "text"),
    ]
    snap_idx = {
        lst: {it["id"]: it for it in snapshot.get(lst, [])}
        for lst, _ in _HASH_FIELDS
    }
        ...
        for item in candidate.get(list_key, []):
            ...
            if compute_hash(content) != stored:
                ...
                logger.warning(
                    ...
                    f"{'restored' if snap else 'no snapshot'}")
            else:
                verified += 1
...
def run_source_verification(summary, messages_by_id):
    """Set source_verified on pinned key_facts and pinned_memory.
    Returns (passed, failed)."""
    _PINNED = {"metric", "reference", "constraint", "commitment"}
    passed = failed = 0
    def _check(item, text_field):
        nonlocal passed, failed
        if item.get("source_verified") is not None: return
        text = item.get(text_field, "")
        combined = " ".join(
            messages_by_id.get(sid, "")
        ...
        if text and text in combined:
            ...
            logger.warning(f"Source verify failed: {text_field}='{text[:60]}'")
    for fact in summary.get("key_facts", []):
        if fact.get("category") in _PINNED: _check(fact, "fact")

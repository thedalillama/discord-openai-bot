...
async def summarize_channel(channel_id, batch_size=None, progress_fn=None):
    ...
    Runs the full cluster pipeline: UMAP + HDBSCAN clustering →
    per-cluster Gemini summarization → cross-cluster overview →
    classify → dedup → save.

    batch_size is accepted but ignored — the cluster pipeline processes
    all messages on each run. Retained for API compatibility.
    """
